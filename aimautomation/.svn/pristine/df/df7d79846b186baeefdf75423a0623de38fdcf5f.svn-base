...
class ApiTest(unittest.TestCase):
    ...
    def setUp(self):
        payload = {"v":self.version, "method":"login", "username":"admin", "password":"password"}
    ...
    def tearDown(self):
        payload = {"v":self.version, "method":"logout", "token":self.token}
        response = self.send_get_request(payload)
        self.confirm_success(response.content)
    
    def test_can_get_devices(self):
        payload = {"v":self.version, "token":self.token, "method":"get_devices", "show_all":"1"}
        response = self.send_get_request(payload)
        self.confirm_success(response.content)
        tree = self.get_element_tree(response.content)
        devices = tree.findall(".//device")
        for device in devices:
            self.devices_dict[device.find("d_id").text] = device.find("d_type").text
        self.assertTrue(len(self.devices_dict) > 0)
    
    def test_can_get_channels(self):
        payload = {"v":self.version, "token":self.token, "method":"get_channels"}
        response = self.send_get_request(payload)
        self.confirm_success(response.content)
        tree = self.get_element_tree(response.content)
    ...
    def test_can_get_presets(self):
        ...
        payload = {"v":self.version, "token":self.token, "method":"get_presets"}
        response = self.send_get_request(payload)
        self.confirm_success(response.content)
        tree = self.get_element_tree(response.content)
        presets = tree.findall(".//connection_preset")
        for preset in presets:
            self.presets_list.append(preset.find("cp_id").text)
    ...
    def test_connect_and_disconnect_channel(self):
        ...
        for device in self.devices_dict:
            if self.devices_dict[device] == "rx":
                receivers.append(device)
        for receiver in receivers:
            for channel in self.channels_list:
                payload = {"v":self.version, "method":"connect_channel", "token":self.token, "c_id":channel, "rx_id":receiver, "exclusive":"1"}
                response = self.send_get_request(payload)
                self.confirm_success(response.content)
                time.sleep(3)
                payload = {"v":self.version, "method":"disconnect_channel", "token":self.token, "rx_id":receiver}
                response = self.send_get_request(payload)
                self.confirm_success(response.content)
                time.sleep(3)
    ...
    def test_connect_and_disconnect_preset(self):
        ...
        for preset in self.presets_list:
            payload = {"v":self.version, "method":"connect_preset", "token":self.token, "id":preset, "force":"1"}
            response = self.send_get_request(payload)
            self.confirm_success(response.content)
            time.sleep(3)
            payload = {"v":self.version, "method":"disconnect_preset", "token":self.token, "id":preset}
            response = self.send_get_request(payload)
            self.confirm_success(response.content)
            time.sleep(3)
    ...
    def test_create_and_delete_preset(self):
        ...
        for device in self.devices_dict:
            if self.devices_dict[device] == "rx":
                receivers.append(device)
        payload = {"v":str(int(self.version)+1), "method":"create_preset", "token":self.token, "name":"test_preset","pairs":self.channels_list[0]+"-"+receivers[0], "allowed":"vse"}
        response = self.send_get_request(payload)
        self.confirm_success(response.content)
        self.test_can_get_presets()
        ...
        pid = pid[0]
        payload = {"v":str(int(self.version)+1), "method":"delete_preset", "token":self.token, "id":pid}
        response = self.send_get_request(payload)
        self.confirm_success(response.content)
        self.test_can_get_presets()
    ...
    def send_get_request(self, payload):
        r = requests.get("http://%s/api" %self.base_url, params = payload,)
        self.assertTrue(r.status_code.ok)
        return r
