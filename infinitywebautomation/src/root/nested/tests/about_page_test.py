...
class AboutPageTest(BaseInfinityRegressionTest):
    ...
    def test_mac_addresses_are_hex_numbers(self):
        ...
        for mac in about_page.get_macs():
            self.assertRegex(mac, pattern)

    def test_version_numbers_are_correct_for_unit(self):
        home_page = HomePage(self._driver, self._wait)
        about_page = home_page.open_about_page()
        self.assertTrue(about_page.get_main_system_version_number()
                        in self._version)
        self.assertTrue(about_page.get_backup_system_version_number()
                        in self._version)
        self.assertTrue(about_page.get_boot_system_version_number()
                        in self._version)
    ...
    def test_board_revision_correct_for_unit(self):
        ...
        if parameter_singleton["device"] == "RX2s":
            self.assertTrue(about_page.get_board_revision() in rx2s)
        elif parameter_singleton["device"] == "TX2s":
            self.assertTrue(about_page.get_board_revision() in tx2s)
        elif parameter_singleton["device"] == "TX2b":
            self.assertTrue(about_page.get_board_revision() in tx2b)
        elif parameter_singleton["device"] == "TX2v":
            self.assertTrue(about_page.get_board_revision() in tx2v)
        elif parameter_singleton["device"] == "TX2":
            self.assertTrue(about_page.get_board_revision() in tx2)
        elif parameter_singleton["device"] == "RX2":
            self.assertTrue(about_page.get_board_revision() in rx2)
